...
const Hero = () => {
  const [smallDevice] = useState(window.innerWidth < 640);
  useGSAP(() => {
    gsap.to("#cta", {
      delay: 2,
      opacity: 1,
    ...
    <section className="w-full nav-height relative px-14">
      <div className="w-full h-5/6 flex-center flex-col ">
        ...
        <p id="hero-slogan" className="hero-slogan -translate-y-20">
          We focus on creating meaningful experiences that resonate{" "}
      ...
      >
        <a href={"https://www.calendly.com/newralfounder"}>
          <button
            className="primary-button max-sm:text-xs flex-row
          ...
          >
            ...
            <img
              src={tiltedArrow}
              alt="arrow"
              className="md:ml-2 w-[1.5vw] max-sm:hidden"
            />
          </button>
        </a>
        <button className="whitespace-nowrap secondary-button max-sm:text-xs ">
          Learn More
        </button>
      </div>
    </section>
